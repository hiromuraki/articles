...
graph TD
    subgraph Host[宿主机（Ubuntu 24.04）]
        ...
        subgraph DockerCompose[Docker Compose: gitea]
            Gitea[Gitea Server<br>代码托管]
            Runner[Act Runner<br>任务调度器]
        end
        
        subgraph Ephemeral[临时容器]
            Worker[CI Worker<br>具体构建任务执行者]
        end
...
```mermaid
graph TD
    subgraph Host[宿主机: 192.168.100.101]
        ...
        subgraph DockerNet[Docker 网络]
            subgraph BridgeNet[Docker 网桥: gitea_default]
                Gitea[Gitea 容器]
                Runner[Runner 容器]
            end
    ...
    %% 出站代理
    Gitea -.->|通过代理访问外网| Proxy
    Worker -.->|通过代理访问外网| Proxy
    
    %% 网络连接关系
    Worker -.->|动态加入网络（gitea_default）| BridgeNet
```
...
services:
    ...
    runner:
        ...
        environment:
            ...
            # 下面这个 Token 需要先启动 server 获取，稍后处理
...
> ROOT_URL 严禁使用 localhost 或 127.0.0.1，否则 Runner 和 Docker 可能会连接到它们自己内部。建议统一使用局域网固定 IP。

> SSH 测试：`ssh -T -p 2222 git@192.168.100.101`
### 3.4. 配置 ACT Runner
...
在 `Gitea` 创建一个空仓库，并将 `test_repo` 代码推送到 `Gitea` 后即可触发 CI，等待镜像构建即可。
